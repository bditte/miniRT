...
/*   Updated: 2020/11/20 18:02:40 by bditte           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_stradd(char **dst, char *str)
{
	...
	if (!(res = malloc(ft_strlenc(*dst, 0) + ft_strlenc(str, 0) + 1)))
		return (NULL);
	len = 0;
	while ((*dst)[len])
	{
...
char	*ft_chrmove(char **dst, int c)
{
	...
	index = ft_strlenc(*dst, c);
	if (!(res = malloc(ft_strlenc(*dst, 0) - index + 1)))
		return (NULL);
	len = ft_strlenc(*dst, 0) - index;
	res[len] = 0;
	while (len--)
